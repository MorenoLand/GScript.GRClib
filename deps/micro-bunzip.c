...
extern int write_bunzip_data(bunzip_data *bd, int out_fd, char *outbuf, int len)
{
	...
	for(;;) {
		...
		if(bd->writeCount<0) {
			if(len && bd->outbufPos) {
				int copyCount = bd->outbufPos < len ? bd->outbufPos : len;
				memcpy(outbuf + gotcount,bd->outbuf,copyCount);
				gotcount+=copyCount;
				bd->outbufPos-=copyCount;
				if(bd->outbufPos)
					memmove(bd->outbuf,bd->outbuf+copyCount,bd->outbufPos);
				return gotcount;
			}
			return gotcount ? gotcount : bd->writeCount;
		}
...
extern int start_bunzip(bunzip_data **bdp, int src_fd, char *inbuf, int len)
{
    ...
    for(i=0;i<3;i++) if(get_bits(bd,8)!=(unsigned int)"BZh"[i]) return RETVAL_NOT_BZIP_DATA;
